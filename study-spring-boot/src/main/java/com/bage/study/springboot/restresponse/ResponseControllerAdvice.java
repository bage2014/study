...
public class ResponseControllerAdvice implements ResponseBodyAdvice<Object> {
    ...
    public boolean supports(MethodParameter returnType, Class<? extends HttpMessageConverter<?>> aClass) {
        ...
        return !returnType.getGenericParameterType().equals(RestReresponse.class);
    }
    ...
    public Object beforeBodyWrite(Object data, MethodParameter returnType, MediaType mediaType, Class<? extends HttpMessageConverter<?>> aClass, ServerHttpRequest request, ServerHttpResponse response) {
        ...
        if (returnType.getGenericParameterType().equals(String.class) || returnType.getParameterType().equals(String.class)) {
            ...
            try {
                log.error("beforeBodyWrite String data = {}",data);
                return objectMapper.writeValueAsString(new RestReresponse(200, data));
            } catch (JsonProcessingException e) {
                e.printStackTrace();
            }
        }
        // 将原本的数据包装在ResultVO里
        log.error("beforeBodyWrite other data = {}",data);
        return new RestReresponse(200, data);
    }
}