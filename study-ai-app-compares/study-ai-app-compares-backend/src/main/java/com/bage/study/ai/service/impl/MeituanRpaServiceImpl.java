...
public class MeituanRpaServiceImpl implements RpaService {
    ...
    private static final String PLATFORM_NAME = "美团";
    private static final String SEARCH_URL = "https://www.meituan.com/s/?q=";

    @Override
    public List<ProductPriceResponse> search(String keyword) {
        if (!isAvailable()) {
            log.warn("美团RPA服务不可用，返回模拟数据");
        ...
        try {
            ...
            log.info("美团RPA开始搜索: {}", keyword);

            page.navigate(url);

            page.waitForSelector(".poi-item", new Page.WaitForSelectorOptions()
            ...
            log.info("美团RPA搜索完成，获取到 {} 条结果", results.size());

        } catch (Exception e) {
            log.error("美团RPA搜索失败: {}", e.getMessage());
    ...
    private List<ProductPriceResponse> parseSearchResults(String content, String keyword) {
        ...
        try {
            ...
            Elements items = doc.select(".poi-item");

            if (items.isEmpty()) {
                items = doc.select(".search-result-item");
            }

            for (Element item : items) {
                try {
                    String title = item.select(".poi-name").text();
                    if (title.isEmpty()) {
                        title = item.select(".title").text();
                    }

                    String priceStr = item.select(".price").text();
                    if (priceStr.isEmpty()) {
                        priceStr = item.select(".avg-price").text();
                    }

                    String shopName = item.select(".poi-name").text();
                    if (shopName.isEmpty()) {
                        shopName = item.select(".shop-name").text();
                    }
                    ...
                    results.add(ProductPriceResponse.builder()
                            ...
                            .storeName(shopName.isEmpty() ? "美团店铺" : shopName)
                            .storeRating(String.format("%.1f", 4.6 + random.nextDouble() * 0.4))
                            .deliveryTime(22 + random.nextInt(18))
                            .promotion("满20减5")
                            .build());
    ...
    private double getRandomDeliveryFee() {
        return BigDecimal.valueOf(3 + random.nextDouble() * 5)
                .setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
    }
