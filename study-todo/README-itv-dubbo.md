...
![Dubbo 架构角色图](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3995983761/p70739.png)
...
![Dubbo 架构分层图](https://images2017.cnblogs.com/blog/1147548/201709/1147548-20170928141450169-1251868962.png)
...
### 4.3 RPC 协议与 HTTP 协议比对

#### 4.3.1 基本概念

- **RPC 协议**：远程过程调用协议，允许程序调用另一个地址空间（通常是共享网络的另一台机器上）的过程或函数，而不需要程序员显式编码这个远程调用的细节。
- **HTTP 协议**：超文本传输协议，是一种用于分布式、协作式和超媒体信息系统的应用层协议，是万维网数据通信的基础。

#### 4.3.2 优缺点比对

| 特性 | RPC 协议 | HTTP 协议 |
|------|---------|-----------|
| **性能** | 高性能，基于 TCP 长连接，序列化效率高 | 相对较低，基于 HTTP 1.1 短连接（HTTP 2.0 支持长连接），序列化效率较低 |
| **易用性** | 需要特定的 RPC 框架支持，使用相对复杂 | 基于标准 HTTP 协议，使用简单，可通过浏览器直接访问 |
| **跨语言** | 部分 RPC 框架支持跨语言，但可能存在兼容性问题 | 天然支持跨语言，任何支持 HTTP 的语言都可以使用 |
| **可读性** | 二进制传输，可读性差，调试困难 | 文本传输（JSON/XML），可读性好，调试方便 |
| **安全性** | 部分 RPC 框架内置安全机制，如 Dubbo 支持 Token 验证、IP 白名单等 | 可利用 HTTPS 加密传输，支持各种认证机制 |
| **生态** | 生态相对单一，主要依赖特定 RPC 框架 | 生态丰富，有大量的工具和库支持 |
| **适用场景** | 内部服务调用，对性能要求较高的场景 | 外部接口，浏览器调用，对性能要求不高的场景 |

#### 4.3.3 技术实现对比

| 技术点 | RPC 协议 | HTTP 协议 |
|--------|---------|-----------|
| **传输层** | 通常基于 TCP 协议 | 基于 TCP 协议 |
| **连接方式** | 通常使用长连接，减少连接建立开销 | HTTP 1.1 使用短连接，HTTP 2.0 支持长连接 |
| **序列化方式** | 支持多种序列化方式，如 Hessian2、Protobuf、JSON 等 | 通常使用 JSON、XML 等文本序列化方式 |
| **调用方式** | 类似本地方法调用，通过接口直接调用 | 通过 HTTP 请求（GET、POST 等）调用 |
| **服务发现** | 通常内置服务发现机制，如 Dubbo 支持 Zookeeper、Nacos 等注册中心 | 需要额外的服务发现机制，如 Eureka、Consul 等 |

#### 4.3.4 选择建议

1. **内部服务调用**：优先选择 RPC 协议，如 Dubbo 协议，以获得更高的性能和更简洁的调用方式。
2. **外部接口**：优先选择 HTTP 协议，如 RESTful API，以获得更好的跨语言支持和易用性。
3. **混合场景**：可以根据具体场景选择合适的协议，如内部服务使用 RPC 协议，外部接口使用 HTTP 协议。
4. **性能要求**：对性能要求较高的场景，如高频调用、大数据量传输，建议使用 RPC 协议。
5. **跨语言需求**：如果需要支持多种编程语言，建议使用 HTTP 协议或支持跨语言的 RPC 框架。
...
// 应用配置
ApplicationConfig application = new ApplicationConfig();
application.setName("consumer-app");
...
method.setTimeout(3000);
method.setRetries(0);
reference.setMethods(Collections.singletonList(method));
...
<beans xmlns="http://www.springframework.org/schema/beans"
    ...
    <!-- 服务配置 -->
    <dubbo:service interface="com.example.UserService" ref="userService" timeout="5000">
        <!-- 方法级配置 -->
...
<beans xmlns="http://www.springframework.org/schema/beans"
    ...
    <!-- 引用配置 -->
    <dubbo:reference id="userService" interface="com.example.UserService" timeout="5000" retries="2">
        <!-- 方法级配置 -->
...
```java
// 应用配置（通常在 application.properties 或 application.yml 中配置）
// spring.application.name=provider-app

// 注册中心配置（通常在 application.properties 或 application.yml 中配置）
// dubbo.registry.address=zookeeper://127.0.0.1:2181

// 协议配置（通常在 application.properties 或 application.yml 中配置）
...
public class UserController {
    ...
    @DubboReference(timeout = 5000, retries = 2)
