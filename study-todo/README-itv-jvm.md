...
G1 收集器的垃圾回收过程主要包括以下几个阶段：

1. **初始标记（Initial Mark）**：标记GC Roots直接关联的对象，此阶段需要STW（Stop The World），但时间很短。
2. **并发标记（Concurrent Mark）**：从GC Roots开始，并发标记所有可达对象，此阶段与用户线程并行执行，不会STW。
3. **最终标记（Final Mark）**：处理并发标记阶段产生的引用变更，此阶段需要STW，但时间较短。
4. **筛选回收（Live Data Counting and Evacuation）**：根据停顿时间目标，选择部分区域进行回收，此阶段需要STW。

#### 13.8.2 核心步骤解释

**1. 初始标记（Initial Mark）**
- **工作原理**：暂停所有用户线程，标记从GC Roots直接可达的对象
- **时间开销**：非常短，通常只有几毫秒
- **执行线程**：GC线程
- **目标**：建立初始的对象标记集合

**2. 并发标记（Concurrent Mark）**
- **工作原理**：恢复用户线程执行，同时GC线程并发标记所有可达对象
- **时间开销**：较长，但与用户线程并行执行，不影响应用响应
- **执行线程**：GC线程与用户线程并发执行
- **目标**：标记所有存活的对象

**3. 最终标记（Final Mark）**
- **工作原理**：暂停所有用户线程，处理并发标记阶段产生的引用变更（如插入到写屏障日志中的引用）
- **时间开销**：较短，通常只有几十毫秒
- **执行线程**：GC线程
- **目标**：确保所有存活对象都被正确标记

**4. 筛选回收（Live Data Counting and Evacuation）**
- **工作原理**：暂停所有用户线程，根据每个区域的存活对象比例和回收价值，选择部分区域进行回收，将存活对象复制到新的区域
- **时间开销**：根据停顿时间目标动态调整，通常在100-200毫秒以内
- **执行线程**：GC线程
- **目标**：回收垃圾对象，压缩存活对象，减少内存碎片
...
G1 收集器的混合回收机制是其核心特性之一，它允许同时回收新生代和老年代的部分区域：

1. **新生代回收**：当新生代区域满时，触发Young GC，只回收新生代区域
2. **混合回收**：当老年代占用比例达到阈值时，触发Mixed GC，同时回收新生代和部分老年代区域
3. **Full GC**：当混合回收无法满足内存需求时，触发Full GC，回收整个堆内存
...
ZGC 收集器的垃圾回收过程主要包括以下几个阶段：

1. **并发标记（Concurrent Mark）**：从GC Roots开始，并发标记所有可达对象，此阶段与用户线程并行执行，不会STW。
2. **并发预备重定位（Concurrent Prepare for Relocation）**：选择部分区域进行重定位，此阶段与用户线程并行执行，不会STW。
3. **并发重定位（Concurrent Relocate）**：将选中区域的存活对象复制到新的区域，此阶段与用户线程并行执行，不会STW。
4. **并发清理（Concurrent Cleanup）**：清理重定位后产生的空区域，此阶段与用户线程并行执行，不会STW。

#### 13.9.2 核心步骤解释

**1. 并发标记（Concurrent Mark）**
- **工作原理**：从GC Roots开始，并发标记所有可达对象，使用着色指针技术标记对象的状态
- **时间开销**：较长，但与用户线程并行执行，不影响应用响应
- **执行线程**：GC线程与用户线程并发执行
- **目标**：标记所有存活的对象

**2. 并发预备重定位（Concurrent Prepare for Relocation）**
- **工作原理**：选择部分区域进行重定位，计算重定位的目标地址
- **时间开销**：较短，与用户线程并行执行
- **执行线程**：GC线程与用户线程并发执行
- **目标**：为后续的重定位操作做准备

**3. 并发重定位（Concurrent Relocate）**
- **工作原理**：将选中区域的存活对象复制到新的区域，使用读屏障确保用户线程可以正确访问正在重定位的对象
- **时间开销**：较长，但与用户线程并行执行，不影响应用响应
- **执行线程**：GC线程与用户线程并发执行
- **目标**：回收垃圾对象，压缩存活对象，减少内存碎片

**4. 并发清理（Concurrent Cleanup）**
- **工作原理**：清理重定位后产生的空区域，将这些区域标记为可用
- **时间开销**：较短，与用户线程并行执行
- **执行线程**：GC线程与用户线程并发执行
- **目标**：回收空区域，提高内存利用率
...
+----------------------------------------+
                  ...
                  v
+----------------------------------------+
|  并发预备重定位（Concurrent Prepare for Relocation） |
|      - 与用户线程并行，选择重定位区域   |
+----------------------------------------+
                  |
                  v
...
- **工作原理**：利用指针的空闲位存储对象的标记状态
- **优势**：无需额外的标记位存储空间，提高标记效率
- **应用**：在并发标记和并发重定位阶段使用

**2. 读屏障（Load Barrier）**
- **工作原理**：在用户线程读取对象引用时插入屏障代码，处理正在重定位的对象
- **优势**：实现并发重定位，避免STW
- **应用**：在并发重定位阶段使用

**3. 基于Region的内存管理**
- **工作原理**：将堆内存划分为多个大小相等的Region（2MB-32MB）
- **优势**：灵活选择部分Region进行回收，控制回收时间
- **应用**：在整个垃圾回收过程中使用

**4. 并发执行**
- **工作原理**：几乎所有阶段都与用户线程并发执行
- **优势**：极低的停顿时间，通常在1毫秒以内
- **应用**：在整个垃圾回收过程中使用
