  ...
  <div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100">
    <div class="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md animate-fade-in">
      ...
      <div class="text-center mb-6">
        ...
        <p class="text-gray-500 text-sm mt-1">传承家族记忆</p>
      </div>
        ...
        <div class="flex items-center justify-between">
          <label class="flex items-center cursor-pointer">
            <input 
              ...
              class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-offset-0"
            />
            <span class="ml-2 text-sm text-gray-600">记住我</span>
          </label>
          <a href="#" class="text-sm text-blue-600 hover:text-blue-700 transition-colors">忘记密码?</a>
        </div>
...
const login = async () => {
  ...
  try {
    ...
    userStore.showToast('登录成功，欢迎回来！', 'success');
    router.push('/home');
  } catch (error) {
    userStore.showToast(userStore.error || '登录失败，请检查邮箱和密码', 'error');
  } finally {
    loading.value = false;
  }
};
