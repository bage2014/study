...
test('home page has feature cards', async ({ page }) => {
  ...
  // Expect feature cards to be present
  await expect(page.locator('.card')).toHaveCount(6);
  await expect(page.locator('.card').nth(0)).toContainText('家族管理');
  await expect(page.locator('.card').nth(1)).toContainText('成员管理');
  await expect(page.locator('.card').nth(2)).toContainText('家族树');
  await expect(page.locator('.card').nth(3)).toContainText('成员大事件');
  await expect(page.locator('.card').nth(4)).toContainText('家族地理位置');
  await expect(page.locator('.card').nth(5)).toContainText('AI关系分析');
});
```
